...
void openSnakeLMenu(MCUFRIEND_kbv);
void printSnakeLMenu(MCUFRIEND_kbv);
void printSnakeLPixel(int, int, String, MCUFRIEND_kbv);
...
//         this->y = y;
//     } void draw(){
//         printSnakeLPixel(this->x, this->y, this->state, tft);
//     } void erase(){
//         printSnakeLPixel(this->x, this->y, empty_id, tft);
//     }
// };
...
class SnakeL{
    public:
        ...
        // DirectionL next_dir = STOP;
        // DirectionL cur_dir = STOP;
        ...
        SnakeL(){}
        SnakeL(int x, int y, MCUFRIEND_kbv tft){
        ...
        void printSnake(){
            ...
            Serial.println("]\n " + String(freeMemory()) +  " BYTE FREE MEMORY LEFT\n" + String(doesLenChanged));
...
void openSnakeLMenu(MCUFRIEND_kbv tft){
    tft.fillScreen(DEFAULT_SNAKE_MENU_COLOR);

    String r_key = ""; // bilmiyorum

    SnakeL snake(5, 5, tft);

    printSnakeLMenu(tft); // mavi çizgiler
    printSnakeLPixel(snake.head.pixel.x, snake.head.pixel.x, snake_id, tft);
...
void printSnakeLMenu(MCUFRIEND_kbv tft){ // ızgara çizdirme
...
void printSnakeLPixel(int x, int y, int status, MCUFRIEND_kbv tft){
    x++;
    y++;
